      ...
      <div className="container max-w-6xl mx-auto text-center relative z-10">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
          {plans.map((plan, idx) => (
            <Card
              key={idx}
              className={`relative flex flex-col text-left overflow-hidden shadow-xl bg-card border-none hover:shadow-2xl transition-all duration-300 hover:-translate-y-1 ${plan.comingSoon ? 'blur-sm pointer-events-none opacity-70' : ''
                }`}
            >
              {plan.comingSoon && (
